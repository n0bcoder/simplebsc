...
print('Masukan Contract Address:')
fkarak = input()
#hapus semua karakter
karakh = re.sub(r'[^a-zA-Z0-9]','',fkarak)
#hapus huruf
combor = karakh.replace('one', '1').replace('two', '2').replace('three', '3').replace('four', '4').replace('five', '5').replace('six', '6').replace('seven', '7').replace('eight', '8').replace('nine', '9').replace('ten', '10')
...
sender_address = akun.akun
 
#Contract Address of Token we want to buy
tokenToBuy = web3.toChecksumAddress(micin)
spend = web3.toChecksumAddress('0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c')  #wbnb#weth
...
'gas': 1000000,
'gasPrice': web3.toWei((gvvwei),'gwei'),
'nonce': nonce,
})
signed_txn = web3.eth.account.sign_transaction(pancakeswap2_txn, private_key=akun.private)
...
while True:
	...
	if block == 1:
		...
		print('Mohon Tunggu sedang di proses')

telusur = 'https://bscscan.com/tx/' #telusuritx
print(telusur+rs)